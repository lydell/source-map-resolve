...
const code = {
  ...
  base64: u2(
    "data:application/json;base64," +
      "eyJtYXBwaW5ncyI6IkFBQUEiLCJzb3VyY2VzIjpbImZvby5qcyJdLCJzb3VyY2VzQ29udGVudCI6WyLkuK3mlofwn5iKIl0sIm5hbWVzIjpbXX0="
  ), // jshint ignore:line
...
describe("resolveSourceMap", () => {
  ...
  it("resolves fileRelative", async () => {
    const result = await resolveSourceMap(
      code.fileRelative,
      codeUrl,
      () => map.simpleString
    );
  ...
  it("resolves base64", async () => {
    const result = await resolveSourceMap(code.base64, codeUrl, (x) => {
      throw x;
    });
    expect(result).toStrictEqual({
      sourceMappingURL:
  ...
  it("resolves absolute with [simpleString]", async () => {
    const result = await resolveSourceMap(code.absolute, codeUrl, () => [
      map.simpleString,
    ]);
    expect(result).toStrictEqual({
      sourceMappingURL: "https://foo.org/foo.js.map",
...
describe("resolveSources", () => {
  ...
  it("resolves simple with array", async () => {
    const result = await resolveSources(map.simple, mapUrl, () => [
      "non",
      "string",
    ]);
    expect(result).toStrictEqual({
...
describe("resolve", () => {
  ...
  it("resolves absolute with simpleString", async () => {
    const result = await resolve(code.absolute, codeUrl, () => [
      map.simpleString,
    ]);
    expect(result).toStrictEqual({
      sourceMappingURL: "https://foo.org/foo.js.map",
  ...
  it("resolves fileRelative with simple string", async () => {
    const result = await resolve(code.fileRelative, codeUrl, () => [
      map.simpleString,
    ]);
    expect(result.sourcesResolved).toStrictEqual([
      "http://example.com/a/b/c/foo.js",
  ...
  describe("mapUrl = example.com", () => {
    ...
    it("resolves null with sourceRoot map and a path in an option", async () => {
      const result = await resolve(null, mapUrl, readMap(map.sourceRoot), {
        sourceRoot: "/static/js/",
      });
      expect(result.sourcesResolved).toStrictEqual([
        "http://example.com/static/js/foo.js",
